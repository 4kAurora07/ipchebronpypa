...
export function Services() {
  return (
    <section id="services" className="relative py-24 md:py-36 bg-[color:var(--color-cream)]">
      <div className="mx-auto max-w-5xl px-6 md:px-10">
        <div className="text-center max-w-2xl mx-auto">
          ...
          <p className="mt-5 font-mal text-foreground/75">എല്ലാവർക്കും സ്വാഗതം</p>
        </div>

        <div className="mt-16 bg-[color:var(--color-ivory)] border border-border/80 rounded-sm shadow-[0_30px_80px_-40px_rgba(31,27,23,0.25)]">
          {services.map((s, i) => (
            <div
              key={i}
              className={`group grid grid-cols-12 items-center gap-4 px-6 md:px-10 py-6 md:py-7 transition-colors hover:bg-[color:var(--color-cream)]/50 ${
                i !== 0 ? "border-t border-border/70" : ""
              }`}
            >
              <div className="col-span-12 md:col-span-3">
                <div className="eyebrow text-gold">{s.day}</div>
              </div>
